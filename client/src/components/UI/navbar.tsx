...
const Navbar: React.FC<NavbarProps> = ({
    ...
    return (
        <header
                className={cn(
                    'w-full py-4 transition-shadow duration-300 bg-transparent backdrop-blur-md ',
                    isSticky && 'sticky top-0 z-50',
                    
                    className
                )}
        >
            <div className={cn('container mx-auto flex items-center justify-between', containerClassName)}>
                {/* Logo */}
                    ...
                    {navItems.map((item) => (
                        <div key={item.label} className="relative group" ref={item.children ? dropdownRef : undefined}>
                            {item.children ? (
                                <>
                                    <button
                                        onMouseEnter={() => setOpenDropdown(openDropdown === item.label ? null : item.label)}
                                        ...
                                        className={cn(
                                            ...
                                            activeTab === item.label || (item.children && item.children.some(child => activeTab === child.label)) ? 'font-bold text-purple-900 dark:text-purple-400' : '',
                                            navItemClassName
                                        )}
                                        aria-haspopup="true"
                                    ...
                                    >
                                        {item.label}
                                        <AiOutlineDown 
                                            className={`ml-1 w-3.5 h-3.5 mt-1 font-bold transition-transform duration-300 ease-in-out ${openDropdown === item.label ? 'rotate-180' : ''}`}
                                        />
                                    </button>
                                        ...
                                        <div className="absolute left-0 mt-2 w-56 bg-white dark:bg-gray-800 shadow-xl rounded-md py-1.5 z-50 border border-gray-100 dark:border-gray-700 transform transition-all duration-200 origin-top-left">
                                            {item.children.map((child, index) => (
                                                <React.Fragment key={child.href || child.label}>
                                                    ...
                                                    >
                                                        {child.icon && <span className="mr-2.5">{child.icon}</span>}
                                                        {child.label}
                                                    </Link>
                                ...
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        activeTab === item.label ? 'font-bold text-purple-900 dark:text-purple-400' : '',
                                        navItemClassName
                                    )}
                ...
                <div className='flex items-center justify-end space-x-4'>
                    <ThemeToggle />
                    <button
                        className={cn(`${mobileMenuBreakpoint}:hidden`, 'text-black dark:text-gray-300 focus:outline-none')}
